...
function generatePdf(layout, gap, sW, sH, perSheet, qty, typeName, priceSheet, pricePc, totalCost) {
  ...
  const pw = mm(SHEET_FULL_W), ph = mm(SHEET_FULL_H);
  // Add extra height for pricing info below layout
  const infoH = mm(45);
  const totalH = ph + infoH;
  let oi = 0; const no = () => ++oi; const off = [];
  const ca = no(), pa = no(), pg = no(), cn = no();
  let s = `0.6 0.6 0.6 RG\n0.5 w\n`;
  // Sheet rect
  s += `${mm(0)} ${mm(45)} ${pw.toFixed(2)} ${ph.toFixed(2)} re S\n`;
  // Margin guide
  s += `0.8 0.8 0.8 RG\n[2 2] 0 d\n0.3 w\n${mm(MARGIN_X)} ${(mm(45) + mm(MARGIN_Y)).toFixed(2)} ${mm(PRINT_W).toFixed(2)} ${mm(PRINT_H).toFixed(2)} re S\n[] 0 d\n`;
  ...
  layout.zones.forEach((z, zi) => {
    ...
    for (let r = 0; r < z.rows; r++) for (let cc = 0; cc < z.cols; cc++) {
      ...
      const x = mm(xm), y = mm(45) + mm(ym), w = mm(z.cellW), h = mm(z.cellH);
  ...
  // Pricing info block below
  const ly = mm(38);
  s += `BT\n0 0.4 0.42 rg\n/F1 10 Tf\n${mm(5).toFixed(2)} ${ly.toFixed(2)} Td\n(Nicksell | ${sW}x${sH} mm | ${typeName}) Tj\nET\n`;
  s += `BT\n0.3 0.3 0.3 rg\n/F1 8 Tf\n${mm(5).toFixed(2)} ${(ly - mm(6)).toFixed(2)} Td\n(Na liste: ${perSheet} sht  |  Listov: ${(qty > 0 ? (Math.round(qty/perSheet*10)/10) : 0)}  |  Tirazh: ${qty} sht) Tj\nET\n`;
  s += `BT\n0.3 0.3 0.3 rg\n/F1 8 Tf\n${mm(5).toFixed(2)} ${(ly - mm(12)).toFixed(2)} Td\n(Za list: ${priceSheet} rub  |  Za 1 sht: ${pricePc} rub  |  Itogo: ${totalCost} rub) Tj\nET\n`;
  s += `BT\n0.6 0.6 0.6 rg\n/F1 6 Tf\n${mm(5).toFixed(2)} ${mm(3).toFixed(2)} Td\n(nicksell.ru | +7 996 228-34-34 | sales@nicksell.ru) Tj\nET\n`;

  const fn = no(); let pdf = "%PDF-1.4\n";
  const wr = (id, ct) => { off[id] = pdf.length; pdf += `${id} 0 obj\n${ct}\nendobj\n`; };
  wr(ca, `<< /Type /Catalog /Pages ${pa} 0 R >>`);
  wr(pa, `<< /Type /Pages /Kids [${pg} 0 R] /Count 1 >>`);
  wr(pg, `<< /Type /Page /Parent ${pa} 0 R /MediaBox [0 0 ${pw.toFixed(2)} ${totalH.toFixed(2)}] /Contents ${cn} 0 R /Resources << /Font << /F1 ${fn} 0 R >> >> >>`);
  wr(cn, `<< /Length ${s.length} >>\nstream\n${s}\nendstream`);
  wr(fn, `<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>`);
  const xr = pdf.length; pdf += `xref\n0 ${oi + 1}\n0000000000 65535 f \n`;
  for (let i = 1; i <= oi; i++) pdf += `${String(off[i]).padStart(10, "0")} 00000 n \n`;
  pdf += `trailer\n<< /Size ${oi + 1} /Root ${ca} 0 R >>\nstartxref\n${xr}\n%%EOF`;
  return pdf;
}

function downloadPdf(s, fn) {
