...
function generatePdf(layout, gap, sW, sH, perSheet, qty, typeName, priceSheet, pricePc, totalCost, tiers, activeType) {
  ...
  const infoH = 95 * mmPx; // more space for price table
  ...
  // Margin guide dashed
  ctx.strokeStyle = "#ccc"; ctx.setLineDash([6, 6]);
  ctx.strokeRect(MARGIN_X * mmPx, MARGIN_Y * mmPx, PRINT_W * mmPx, PRINT_H * mmPx);
  ctx.setLineDash([]);
  ...
  layout.zones.forEach((z, zi) => {
    for (let r = 0; r < z.rows; r++) for (let cc = 0; cc < z.cols; cc++) {
      ...
      const y = (z.y + r * (z.cellH + gap)) * mmPx;
      const w = z.cellW * mmPx, h = z.cellH * mmPx;
      ctx.fillStyle = cls[zi % cls.length];
      ctx.fillRect(x, y, w, h);
      ctx.strokeStyle = brd[zi % brd.length]; ctx.lineWidth = 0.5;
      ctx.strokeRect(x, y, w, h);
      cnt++;
      ctx.fillStyle = "#555";
      ctx.font = `${Math.max(10, mmPx * 2.5)}px sans-serif`;
  ...
  const iy = ch + mmPx * 5;
  const fs = mmPx; // base font scale
  ctx.textAlign = "left"; ctx.textBaseline = "top";

  // Title
  ctx.fillStyle = "#0a6668";
  ctx.font = `bold ${fs * 6}px sans-serif`;
  ctx.fillText(`${sW}×${sH} мм  ·  ${typeName}`, mmPx * 5, iy);

  // Stats line 1
  ctx.fillStyle = "#333";
  ctx.font = `${fs * 4.5}px sans-serif`;
  const sheets = qty > 0 && perSheet > 0 ? Math.round(qty / perSheet * 10) / 10 : 0;
  ctx.fillText(`На листе: ${perSheet} шт   ·   Листов: ${sheets}   ·   Тираж: ${qty} шт`, mmPx * 5, iy + fs * 9);

  // Stats line 2
  ctx.font = `bold ${fs * 4.5}px sans-serif`;
  ctx.fillText(`За лист: ${priceSheet} ₽   ·   За 1 шт: ${pricePc} ₽   ·   Итого: ${totalCost} ₽`, mmPx * 5, iy + fs * 15);

  // Separator line
  ctx.strokeStyle = "#ddd"; ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(mmPx * 5, iy + fs * 22); ctx.lineTo(cw - mmPx * 5, iy + fs * 22); ctx.stroke();

  // Price table header
  const ty = iy + fs * 26;
  ctx.fillStyle = "#0a6668";
  ctx.font = `bold ${fs * 4}px sans-serif`;
  ctx.fillText("Тарифная сетка", mmPx * 5, ty);

  // Table
  const colX = [mmPx * 5, mmPx * 40, mmPx * 75, mmPx * 110];
  const tableY = ty + fs * 6;
  const rowH = fs * 5;

  // Table header
  ctx.fillStyle = "#888";
  ctx.font = `bold ${fs * 3}px sans-serif`;
  ctx.fillText("Листов", colX[0], tableY);
  ctx.fillText("Наклеек", colX[1], tableY);
  ctx.fillText("За лист", colX[2], tableY);
  ctx.fillText("За 1 шт", colX[3], tableY);

  // Table rows
  const sheetsForTier = Math.ceil(sheets);
  tiers.forEach((t, i) => {
    const ry = tableY + rowH * (i + 1);
    const isActive = sheetsForTier >= t.minSheets && sheetsForTier <= t.maxSheets && qty > 0;
    ...
    const minS = t.minSheets * perSheet;
    const maxS = t.maxSheets >= 99999 ? "∞" : String(t.maxSheets * perSheet);

    if (isActive) {
      ctx.fillStyle = "rgba(42,160,168,0.08)";
      ctx.fillRect(colX[0] - 4, ry - 2, cw - mmPx * 10 + 8, rowH);
    }

    ctx.font = isActive ? `bold ${fs * 3.2}px sans-serif` : `${fs * 3.2}px sans-serif`;
    ctx.fillStyle = isActive ? "#1a3a3e" : "#666";
    ctx.fillText(lbl, colX[0], ry);
    ctx.fillText(`${minS}–${maxS}`, colX[1], ry);
    ctx.fillText(`${p} ₽`, colX[2], ry);
    ctx.fillStyle = isActive ? "#b8920a" : "#888";
    ctx.font = `bold ${fs * 3.2}px sans-serif`;
    ctx.fillText(`${pps} ₽`, colX[3], ry);
  });
...
export default function StickerCalc() {
  ...
  const handleDownloadPdf = useCallback(() => {
    ...
    const pdf = generatePdf(layout, effectiveGap, numW, numH, perSheet, numQty, sType.label, pricePerSheet, pricePerSticker.toFixed(2), Math.round(totalCost), activeTiers, activeType);
